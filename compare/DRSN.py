...
from tensorflow.keras.layers import Dense, Conv1D, BatchNormalization, Activation, MaxPooling1D
...
def cnn_block(x, out_channels):
    ...
    x = Conv1D(out_channels, kernel_size=3, padding='same', kernel_initializer='he_normal', activation='relu')(x)
    x = Conv1D(out_channels, kernel_size=3, padding='same', kernel_initializer='he_normal', activation='relu')(x)
...
def cw_threshold(inputs):
    ...
    abs_mean = GlobalAveragePooling1D()(abs_x)  # C
...
def cs_threshold(inputs):
    ...
    [w, channels] = inputs.get_shape().as_list()[1:]
    ...
    #thres = K.tile(thres, (1, w, 1, channels))  # W*H*C
...
def creat_model(input_shape, class_num, model_type):
    ...
    if model_type == 'cnn':
        ...
        #print("cs_block",x.shape)
        x = cnn_block(x, 8)
        # print("cnn_block",x.shape)
        x = cs_threshold(x)
...
def train(input_shape, x_train, y_train, batch_size, epoch, model_type):
...
if __name__ == '__main__':
    ...
    input_shape, x_train, y_train, x_test, y_test = (1024,1), x_train,y_train,x_test,y_test
    start = time.clock()
